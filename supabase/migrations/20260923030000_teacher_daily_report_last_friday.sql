-- =========================================================================
-- Informe docente: no evaluar el último viernes de cada mes (2026-09-23)
--
-- Confirmado por el colegio: cada último viernes de mes no se imparte clases
-- porque los docentes reciben talleres. Sin esto, el informe de las 3:00 pm
-- marcaría a todo el personal como "no pasó lista" ese día. Solo reemplaza
-- private.teacher_daily_report (create or replace, mismo cuerpo que
-- 20260922010000 más esta regla); el cron y el envío no cambian. Idempotente.
-- =========================================================================
...
begin
  ...
  -- Último viernes del mes: no hay clases, los docentes reciben talleres.
  if v_dow = 5 and extract(month from p_date + 7) <> extract(month from p_date) then
    return v_titulo || E'\nÚltimo viernes del mes: no hay clases (talleres docentes). No se evalúa.';
  end if;
