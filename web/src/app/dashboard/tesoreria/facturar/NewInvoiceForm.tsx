...
export default function NewInvoiceForm({ schoolId, authorProfileId, families, concepts }: NewInvoiceFormProps) {
  ...
  const taxAmount = applyTax ? Math.round(Number(amount || 0) * 0.18 * 100) / 100 : 0
  const totalAmount = Math.round((Number(amount || 0) + taxAmount) * 100) / 100
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-6">
      <div className="rounded-2xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 p-5 space-y-4">
        ...
        <div className="rounded-xl bg-slate-50 dark:bg-slate-800/50 p-4 flex justify-between text-sm">
