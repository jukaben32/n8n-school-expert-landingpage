...
/**
 * Descargar el listado de cuentas por cobrar (Excel/Google Sheets vía CSV) o
 * imprimirlo (el diálogo de impresión del navegador permite "Guardar como
 * PDF"). Mismo patrón que ExportStudentsButton.tsx en Estudiantes.
 *
 * Todo ocurre en el navegador con las filas ya filtradas que
 * ReceivablesTable le pasa -- respeta la búsqueda y los filtros de nivel/curso
 * que el usuario ya aplicó en pantalla, no vuelve a consultar nada.
 */
export default function ExportReceivablesButton({
  filas,
  nivel,
}: {
  ...
  function nombreArchivo(extension: string) {
    const nivelSlug = nivel && nivel !== 'todos'
      ? nivel
          .toLowerCase()
          .normalize('NFD')
          .replace(/[̀-ͯ]/g, '')
          ...
          .replace(/(^-|-$)/g, '')
      : 'todos'
    return `cuentas-por-cobrar-${nivelSlug}-${new Date().toISOString().slice(0, 10)}.${extension}`
  }
